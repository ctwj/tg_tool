...
pub fn build_router(state: AppState) -> Router {
    ...
    let api_routes = Router::new()
        ...
        .route("/auth/me", get(handlers::auth::get_me).put(handlers::auth::update_me))
        .route("/auth/token", post(handlers::auth::generate_api_token))
        // Status (public)
        .route("/status", get(handlers::misc::system_status))
        // Clients
        .route("/clients", get(handlers::client::list_clients).post(handlers::client::add_client))
        .route("/clients/{id}",
            delete(handlers::client::remove_client)
            .get(handlers::client::get_client_status))
        ...
        // Rules
        .route("/rules", get(handlers::rule::list_rules).post(handlers::rule::create_rule))
        .route("/rules/{id}",
        ...
        .route("/rules/{id}/messages", get(handlers::rule::get_rule_messages))
        // Collectors
        .route("/collectors", get(handlers::collector::list_collectors).post(handlers::collector::create_collector))
        .route("/collectors/histories", get(handlers::collector::list_histories))
        .route("/collectors/{id}",
            get(handlers::collector::get_collector)
            .put(handlers::collector::update_collector)
            .delete(handlers::collector::delete_collector))
        .route("/collectors/{id}/toggle", put(handlers::collector::toggle_collector))
        .route("/collectors/{id}/fetch", post(handlers::collector::fetch_history))
        // Push
        ...
        // Files
        .route("/files", get(handlers::file::list_files).post(handlers::file::upload_file))
        .route("/files/{id}", delete(handlers::file::delete_file))
        .route("/files/download/{filename}", get(handlers::file::download_file))
        // Options
        .route("/options", get(handlers::option::get_options).put(handlers::option::update_options));
